...
export default function ContactForm() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setState({ sending: false, ok: false, error: err.message || "Fehler" });
    }
  }
    ...
    <form className="space-y-5" onSubmit={onSubmit}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            type="text"
            className="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-sm text-slate-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-brand/60 focus:border-brand"
            placeholder="z. B. 23795 Bad Segeberg"
          />
        </div>
      </div>
      ...
      <div className="flex items-start gap-2 text-xs text-slate-600">
        ...
        <label htmlFor="privacy">
          ...
          gelesen und akzeptiere sie. Ich bin damit einverstanden, dass meine
  ...
  <aside className="relative rounded-2xl bg-brand/95 text-slate-50 p-6 md:p-7 shadow-lg space-y-5">
  {/* Desktop: Figur schwebt oben rechts */}
  <div className="pointer-events-none absolute -top-10 -right-3 hidden md:block">
    <Image
      src="/mn/contact-support.png"   // <- dein PNG (transparent)
  ...
  <div className="pt-2 flex flex-col gap-2 text-sm">
    ...
    <a
      ...
      className="inline-flex items-center justify-center px-4 py-2 rounded-full border border-slate-200 text-slate-50 hover:bg-white/10 transition"
    >
